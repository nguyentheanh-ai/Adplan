...
async function readJson<T>(response: Response) {
  ...
  if (!response.ok) {
    throw new Error(payload.error ?? "Meta API trả về lỗi.");
  }
...
export function MetaDashboard({ compact = false }: { compact?: boolean }) {
  ...
  const loadCampaignsForAccount = useCallback(async (adAccountId: string) => {
    ...
    setError(null);
    try {
      const campaignData = await fetch(`/api/meta/campaigns?ad_account_id=${encodeURIComponent(adAccountId)}`).then(
        (response) => readJson<MetaCampaign[]>(response)
      );
  ...
  const loadMeta = useCallback(async (preferredAdAccountId?: string) => {
    setLoading(true);
    setError(null);
    try {
      ...
      const nextSelected = preferredAdAccountId || nextAccounts[0]?.id || "";
      setAccounts(nextAccounts);
      setSelectedAdAccountId(nextSelected);

      if (nextSelected) {
        const campaignData = await fetch(`/api/meta/campaigns?ad_account_id=${encodeURIComponent(nextSelected)}`).then(
          (response) => readJson<MetaCampaign[]>(response)
        );
        setCampaigns(campaignData ?? []);
      } else {
    ...
    return (
      <Card className="mt-8 overflow-hidden rounded-2xl p-0">
        <div className="flex items-center justify-between border-b border-outline-variant px-6 py-6">
          ...
          <Button disabled={loading} onClick={() => loadMeta(selectedAdAccountId)} variant="secondary">
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 rounded-2xl border border-primary/10 bg-white p-5 custom-shadow md:flex-row md:items-center md:justify-between">
        ...
        <Button disabled={loading} onClick={() => loadMeta(selectedAdAccountId)} variant="ai">
      ...
      <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <Card className="rounded-2xl p-6">
          ...
          <p className="mb-4 text-sm leading-6 text-on-surface-variant">
            Chọn tài khoản muốn dùng để xem campaign và tạo campaign PAUSED. ID quảng cáo được hiển thị để bạn kiểm tra đúng tài khoản.
          </p>
          {accounts.length ? (
        ...
        <Card className="rounded-2xl p-6">
          ...
          <form action={createCampaign} className="space-y-4">
            ...
            <div className="rounded-xl bg-surface-container-low p-4 text-sm leading-6 text-on-surface-variant">
              ...
              <br />
              Campaign luôn được tạo với <strong>status: PAUSED</strong>, <strong>buying_type: AUCTION</strong> và{" "}
              <strong>special_ad_categories: []</strong>. App không tạo campaign ACTIVE.
            </div>
...
function CampaignTable({ campaigns }: { campaigns: MetaCampaign[] }) {
  if (!campaigns.length) {
    return <EmptyMetaText text="Chưa có campaign hoặc token chưa có quyền đọc campaign." />;
  }
