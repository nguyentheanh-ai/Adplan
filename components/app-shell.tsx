...
import { getActiveNavGroupKey, navGroups } from "@/lib/navigation";
...
export function AppShell({
  ...
  }, [lockedSections, userRole]);

  const activeGroupKey = getActiveNavGroupKey(pathname) ?? "ads";
  const mobileNavItems = useMemo(() => {
    return visibleNavGroups.map((group) => group.items[0]).filter(Boolean);
  }, [visibleNavGroups]);
  ...
  async function signOut() {
    try {
      ...
      // Van dieu huong ve login neu logout tam thoi loi.
    }
  ...
  return (
    <div className="min-h-screen bg-background text-on-background">
      <aside
        className={cn(
          "fixed left-0 top-0 z-50 hidden h-full w-[262px] flex-col border-r border-outline-variant bg-surface-container-lowest transition-transform md:flex",
      ...
      >
        <div className="px-5 py-6">
          <Link href="/" className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-[28px] font-extrabold leading-none text-primary">Greezhub x Adplan</h1>
              <p className="mt-1 text-[11px] font-bold uppercase tracking-wide text-outline">Workspace + Ads Shell</p>
            </div>
          </Link>
        </div>
            ...
            {visibleNavGroups.map((group) => {
              ...
              return (
                <Link
                  ...
                  href={primaryHref}
                  className={cn(
                    "flex items-center gap-3 rounded-xl border border-outline-variant px-4 py-4 text-sm font-extrabold text-on-surface-variant transition hover:bg-surface-container",
                    groupActive && "border-primary bg-primary text-white hover:bg-primary"
                  )}
                >
                  <MaterialIcon filled={groupActive} name={group.items[0]?.icon ?? "apps"} />
        ...
        <div className="px-4 py-5">
          <div className="mb-4 rounded-lg border border-outline-variant bg-surface-container-low p-4">
            ...
            <p className="mt-1 text-sm font-bold text-on-surface">Ads backend vẫn giữ tại Adplan AI</p>
            <p className="mt-1 text-xs leading-5 text-on-surface-variant">Shell chỉ điều hướng, không đưa token hay luồng publish sang frontend.</p>
      ...
      <header
        className={cn(
          ...
          sidebarHidden ? "md:w-full" : "md:w-[calc(100%-262px)]"
        )}
      >
        <Link className="flex items-center gap-2 md:hidden" href="/">
          ...
          <span className="font-extrabold text-primary">G x A</span>
        </Link>
        <div className="hidden text-sm font-bold text-primary md:block">Unified Workspace + Ads Operations</div>
      ...
      <main className={cn("min-h-screen transition-all", sidebarHidden ? "md:ml-0" : "md:ml-[262px]", contentClassName)}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 grid h-16 grid-cols-2 border-t border-outline-variant bg-white px-2 md:hidden">
        {mobileNavItems.map((item) => {
          const active = getActiveNavGroupKey(pathname) === (item.href === "/ads" ? "ads" : "workspace");
          return (
