...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ data: payload.report.adInsights, status: payload.status.meta });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Không thể tải Meta Insights.";
