...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ data: payload.report.daily, status: payload.status });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Không thể tải dữ liệu theo ngày.";
