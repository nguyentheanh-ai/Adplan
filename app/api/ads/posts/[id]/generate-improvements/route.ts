...
import { generateImprovedVersions } from "@/lib/ads-posts/analysis";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { posts } = rankCreativePosts(normalizeCreativePerformance(ads));
    ...
    return NextResponse.json({ data: { source: "local-creative-rewrite", versions: generateImprovedVersions(post) } });
  } catch (error) {
